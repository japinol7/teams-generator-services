...
def test_calc_team_1_member(team_names_resource, random_choice_names_mock):
    result = calc_team(
        team_name=TEAM_1_NAME, names=team_names_resource, names_sel=[], n_members=1
    )
...
def test_calc_team_3_members(team_names_resource, random_choice_names_mock):
    result = calc_team(
        team_name=TEAM_1_NAME, names=team_names_resource, names_sel=[], n_members=3
    )
...
def test_calc_team_no_members(team_names_resource, random_choice_names_mock):
    result = calc_team(
        team_name=TEAM_1_NAME, names=team_names_resource, names_sel=[], n_members=0
    )
...
def test_calc_team_not_enough_names():
    result = calc_team(
        team_name=TEAM_1_NAME, names=['Name1', 'Name2'], names_sel=[], n_members=3
    )
...
def test_calc_team_side_effect_names_sel_once(random_choice_names_repeated_mock):
    ...
    result = calc_team(
        team_name=TEAM_1_NAME, names=names, names_sel=['Rei Ayanami'], n_members=2
    )
...
def test_calc_team_2_teams_not_enough_names():
    ...
    result = calc_team(
        team_name=TEAM_1_NAME, names=names, names_sel=names_sel, n_members=3
    )
    result_expected = ERROR_RES_NOT_ENOUGH_CHARACTERS
    assert result == result_expected
